import Signal exposing (Signal, map2, foldp, mailbox, Mailbox, message)
import Signal
import Color exposing (..)
import Graphics.Element exposing (..)
import Graphics.Input exposing (hoverable, button, customButton)
import Text
import Window
import Mouse
import String exposing (padLeft)
import Graphics.Collage exposing (..)
import Html exposing (a, text, toElement)
import Html.Attributes exposing (href, target)

main : Signal Element
main =
  map2 view sig hoveredOn.signal
...
hoveredOn : Signal.Mailbox String
hoveredOn = Signal.mailbox ""
...
customLButton sig =
 customButton (message chan.address (sig - 1))
...
customRestartButton =
 customButton (message chan.address (0))
...
importColor =
  codeText "import Color exposing (..)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address colorMsg) else (Signal.message hoveredOn.address ""))
...
importGraphicsElement =
  codeText "import Graphics.Element exposing (..)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address graphicsElementMsg) else (Signal.message hoveredOn.address ""))
...
importGraphicsCollage =
  codeText "import Graphics.Collage exposing (..)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address graphicsCollageMsg) else (Signal.message hoveredOn.address ""))
...
importText =
  codeText "import Text"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address textMsg) else (Signal.message hoveredOn.address ""))
...
importSignal =
  codeText "import Signal"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalMsg) else (Signal.message hoveredOn.address ""))
...
importKeyboard =
  codeText ("import Keyboard")
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address keyboardMsg) else (Signal.message hoveredOn.address ""))
...
importTime =
  codeText ("import Time")
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address timeMsg) else (Signal.message hoveredOn.address ""))
...
importWindow =
  codeText ("import Window")
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address windowMsg) else (Signal.message hoveredOn.address ""))
...
signalMain1 =
  codeText "main ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalMain1Msg) else (Signal.message hoveredOn.address ""))
...
signalMain2 =
  codeText "  Signal.map2 view Window.dimensions gameState"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalMain2Msg) else (Signal.message hoveredOn.address ""))
...
signalGameState1 =
  codeText "gameState : Signal Game"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalGameState1Msg) else (Signal.message hoveredOn.address ""))

signalGameState2Msg = "Define the specifics of gameState."
signalGameState2 : Element
signalGameState2 =
  codeText "gameState ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalGameState2Msg) else (Signal.message hoveredOn.address ""))
...
signalGameState3 =
  codeText "  Signal.foldp update defaultGame input"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalGameState3Msg) else (Signal.message hoveredOn.address ""))
...
signalDelta1 =
  codeText "delta ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalDelta1Msg) else (Signal.message hoveredOn.address ""))
...
signalDelta2 =
  codeText "  Signal.map inSeconds (fps 35)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalDelta2Msg) else (Signal.message hoveredOn.address ""))
...
signalInput1 =
  codeText "input : Signal Input"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalInput1Msg) else (Signal.message hoveredOn.address ""))

signalInput2Msg = "Defines the specifics of input."
signalInput2 : Element
signalInput2 =
  codeText "input ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalInput2Msg) else (Signal.message hoveredOn.address ""))
...
signalInput3 =
  codeText "  Signal.sampleOn delta <|"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalInput3Msg) else (Signal.message hoveredOn.address ""))
...
signalInput4 =
  codeText "    Signal.map4 Input"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalInput4Msg) else (Signal.message hoveredOn.address ""))
...
signalInput5 =
  codeText "      Keyboard.space"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalInput5Msg) else (Signal.message hoveredOn.address ""))
...
signalInput6 =
  codeText "      (Signal.map .x Keyboard.arrows)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalInput6Msg) else (Signal.message hoveredOn.address ""))
...
signalInput7 =
  codeText "      (Signal.map .y Keyboard.arrows)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalInput7Msg) else (Signal.message hoveredOn.address ""))

signalInput8Msg = "Finally, the change in time is recorded in seconds."
signalInput8 : Element
signalInput8 =
  codeText "      delta"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address signalInput8Msg) else (Signal.message hoveredOn.address ""))
...
gameSize =
  codeText "(gameWidth, gameHeight) = (600, 400)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address gameSizeMsg) else (Signal.message hoveredOn.address ""))
...
playingSize =
  codeText "(halfWidth, halfHeight) = (300, 200)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address playingSizeMsg) else (Signal.message hoveredOn.address ""))
...
typeState =
  codeText "type State = Play | Pause"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address typeStateMsg) else (Signal.message hoveredOn.address ""))
...
aliasBadGuy1 =
  codeText "type alias BadGuy ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address aliasBadGuy1Msg) else (Signal.message hoveredOn.address ""))
...
aliasBadGuy2 =
  codeText "  { x:Float, y:Float, vx:Float, vy:Float }"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address aliasBadGuy2Msg) else (Signal.message hoveredOn.address ""))
...
aliasPlayer1 =
  codeText "type alias Player ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address aliasPlayer1Msg) else (Signal.message hoveredOn.address ""))
...
aliasPlayer2 =
  codeText "  { x:Float, y:Float, vx:Float, vy:Float, lives:Int }"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address aliasPlayer2Msg) else (Signal.message hoveredOn.address ""))
...
aliasGame1 =
  codeText "type alias Game ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address aliasGame1Msg) else (Signal.message hoveredOn.address ""))
...
aliasGame2 =
  codeText "  { state:State, badGuy:BadGuy, player1:Player }"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address aliasGame2Msg) else (Signal.message hoveredOn.address ""))
...
defaultGame1 =
  codeText "defaultGame : Game"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address defaultGame1Msg) else (Signal.message hoveredOn.address ""))

defaultGame2Msg = "Describe specific values for defaultGame."
defaultGame2 : Element
defaultGame2 =
  codeText "defaultGame ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address defaultGame2Msg) else (Signal.message hoveredOn.address ""))

defaultGame3Msg = "The state is set to the value of Pause."
defaultGame3 : Element
defaultGame3 =
  codeText "  { state = Pause"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address defaultGame3Msg) else (Signal.message hoveredOn.address ""))
...
defaultGame4 =
  codeText "  , badGuy = { x=0, y=0, vx=300, vy=300 }"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address defaultGame4Msg) else (Signal.message hoveredOn.address ""))
...
defaultGame5 =
  codeText "  , player1 = { x = 15-halfWidth, y = 0, vx = 0,"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address defaultGame5Msg) else (Signal.message hoveredOn.address ""))
...
defaultGame7 =
  codeText "  }"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address defaultGame7Msg) else (Signal.message hoveredOn.address ""))
...
aliasInput1 =
  codeText "type alias Input ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address aliasInput1Msg) else (Signal.message hoveredOn.address ""))
...
aliasInput2 =
  codeText "  { space : Bool"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address aliasInput2Msg) else (Signal.message hoveredOn.address ""))
...
aliasInput3 =
  codeText "  , dir1 : Int"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address aliasInput3Msg) else (Signal.message hoveredOn.address ""))
...
aliasInput4 =
  codeText "  , dir2 : Int"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address aliasInput4Msg) else (Signal.message hoveredOn.address ""))

aliasInput5Msg = "There is an attribute called delta with a Time value."
aliasInput5 : Element
aliasInput5 =
  codeText "  , delta : Time"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address aliasInput5Msg) else (Signal.message hoveredOn.address ""))

aliasInput6Msg = "Ends the list of attributes for Input."
aliasInput6 : Element
aliasInput6 =
  codeText "  }"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address aliasInput6Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc1 =
  codeText "update : Input -> Game -> Game"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc1Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc2 =
  codeText "update {space,dir1,dir2,delta} ({state,badGuy,"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc2Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc4 =
  codeText "  let lives = if (badGuy `within` player1) then 1"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc4Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc5 =
  codeText "    newState ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc5Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc6 =
  codeText "      if | space                    -> Play"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc6Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc7 =
  codeText "         | player1.lives == 0 -> Pause"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc7Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc8 =
  codeText "         | otherwise              -> state"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc8Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc9 =
  codeText "    newBadGuy ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc9Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc10 =
  codeText "      if state == Pause"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc10Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc11 =
  codeText "         then badGuy"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc11Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc12 =
  codeText "         else updateBadGuy delta badGuy player1"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc12Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc13 =
  codeText "  in"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc13Msg) else (Signal.message hoveredOn.address ""))

updateFunc14Msg = "Game will have its attributes updated."
updateFunc14 : Element
updateFunc14 =
  codeText "    { game |"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc14Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc15 =
  codeText "      state   <- newState,"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc15Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc16 =
  codeText "      badGuy  <- newBadGuy,"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc16Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc17 =
  codeText "      player1 <- updatePlayer delta dir1 dir2"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc17Msg) else (Signal.message hoveredOn.address ""))
...
updateFunc18 =
  codeText "    }"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateFunc18Msg) else (Signal.message hoveredOn.address ""))
...
updateBadGuy1 =
  codeText "updateBadGuy : Time -> BadGuy -> Player ->"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateBadGuy1Msg) else (Signal.message hoveredOn.address ""))
...
updateBadGuy2 =
  codeText "updateBadGuy t ({x,y,vx,vy} as badGuy)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateBadGuy2Msg) else (Signal.message hoveredOn.address ""))
...
updateBadGuy3 =
  codeText "  if | (player1.lives == 0) -> { badGuy | x <- 0,"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateBadGuy3Msg) else (Signal.message hoveredOn.address ""))
...
updateBadGuy4 =
  codeText "     | (badGuy `within` player1) -> physicsUpdate t"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateBadGuy4Msg) else (Signal.message hoveredOn.address ""))
...
updateBadGuy5 =
  codeText "       { badGuy |"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateBadGuy5Msg) else (Signal.message hoveredOn.address ""))
...
updateBadGuy6 =
  codeText "        vx <- stepV vx (badGuy.vx < 0)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateBadGuy6Msg) else (Signal.message hoveredOn.address ""))
...
updateBadGuy7 =
  codeText "        vy <- stepV vy (badGuy.vy < 0 ) "
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateBadGuy7Msg) else (Signal.message hoveredOn.address ""))
...
updateBadGuy8 =
  codeText "       }"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateBadGuy8Msg) else (Signal.message hoveredOn.address ""))
...
updateBadGuy9 =
  codeText "     | otherwise -> physicsUpdate t"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateBadGuy9Msg) else (Signal.message hoveredOn.address ""))
...
updateBadGuy10 =
  codeText "       { badGuy |"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateBadGuy10Msg) else (Signal.message hoveredOn.address ""))
...
updateBadGuy11 =
  codeText "         vx <- stepV vx (x < 25-halfWidth) "
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateBadGuy11Msg) else (Signal.message hoveredOn.address ""))
...
updateBadGuy12 =
  codeText "         vy <- stepV vy (y < 25-halfHeight) "
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateBadGuy12Msg) else (Signal.message hoveredOn.address ""))
...
updateBadGuy13 =
  codeText "       }"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateBadGuy13Msg) else (Signal.message hoveredOn.address ""))
...
updatePlayer1 =
  codeText "updatePlayer : Time -> Int -> Int -> Int -> "
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePlayer1Msg) else (Signal.message hoveredOn.address ""))
...
updatePlayer2 =
  codeText "updatePlayer t dir1 dir2 lives player ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePlayer2Msg) else (Signal.message hoveredOn.address ""))
...
updatePlayer3 =
  codeText "  let player1 = physicsUpdate  t { player | "
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePlayer3Msg) else (Signal.message hoveredOn.address ""))
...
updatePlayer4 =
  codeText "     vy <- toFloat dir2 * 200}"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePlayer4Msg) else (Signal.message hoveredOn.address ""))
...
updatePlayer5 =
  codeText "    alive = if (player.lives - lives < 0)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePlayer5Msg) else (Signal.message hoveredOn.address ""))
...
updatePlayer6 =
  codeText "      then 0"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePlayer6Msg) else (Signal.message hoveredOn.address ""))
...
updatePlayer7 =
  codeText "      else (player.lives - lives)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePlayer7Msg) else (Signal.message hoveredOn.address ""))
...
updatePlayer8 =
  codeText "  in"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePlayer8Msg) else (Signal.message hoveredOn.address ""))
...
updatePlayer9 =
  codeText "    { player1 |"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePlayer9Msg) else (Signal.message hoveredOn.address ""))
...
updatePlayer10 =
  codeText "      y <- clamp (35-halfHeight) (halfHeight-35)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePlayer10Msg) else (Signal.message hoveredOn.address ""))
...
updatePlayer11 =
  codeText "      x <- clamp (35-halfWidth) (halfWidth-35)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePlayer11Msg) else (Signal.message hoveredOn.address ""))
...
updatePlayer12 =
  codeText "      lives <- alive"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePlayer12Msg) else (Signal.message hoveredOn.address ""))

updatePlayer13Msg = "This bracket ends the update for player1."
updatePlayer13 : Element
updatePlayer13 =
  codeText "    }"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePlayer13Msg) else (Signal.message hoveredOn.address ""))
...
updatePhysics1 =
  codeText "physicsUpdate t ({x,y,vx,vy} as obj) ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePhysics1Msg) else (Signal.message hoveredOn.address ""))
...
updatePhysics2 =
  codeText "  { obj |"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePhysics2Msg) else (Signal.message hoveredOn.address ""))
...
updatePhysics3 =
  codeText "    x <- x + vx * t,"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePhysics3Msg) else (Signal.message hoveredOn.address ""))
...
updatePhysics4 =
  codeText "    y <- y + vy * t"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePhysics4Msg) else (Signal.message hoveredOn.address ""))

updatePhysics5Msg = "This brakcet closes the updatePhysics function."
updatePhysics5 : Element
updatePhysics5 =
  codeText "  }"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updatePhysics5Msg) else (Signal.message hoveredOn.address ""))
...
updateNear1 =
  codeText "near k c n ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateNear1Msg) else (Signal.message hoveredOn.address ""))
...
updateNear2 =
  codeText "  n >= k-c && n <= k+c"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateNear2Msg) else (Signal.message hoveredOn.address ""))
...
updateWithin1 =
  codeText "within badGuy player1 ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateWithin1Msg) else (Signal.message hoveredOn.address ""))
...
updateWithin2 =
  codeText "  near player1.x 25 badGuy.x && "
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateWithin2Msg) else (Signal.message hoveredOn.address ""))
...
updateStepV1 =
  codeText "stepV v condition1 condition2 ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateStepV1Msg) else (Signal.message hoveredOn.address ""))
...
updateStepV2 =
  codeText "  if | condition1 -> abs v"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateStepV2Msg) else (Signal.message hoveredOn.address ""))
...
updateStepV3 =
  codeText "     | condition2 -> 0 - abs v"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateStepV3Msg) else (Signal.message hoveredOn.address ""))
...
updateStepV4 =
  codeText"     | otherwise  -> v"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address updateStepV4Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc1 =
  codeText "view : (Int, Int) -> Game -> Element"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc1Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc2 =
  codeText "view (w, h) {state, badGuy, player1} ="
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc2Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc3 =
  codeText "  let lives : Element"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc3Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc4 =
  codeText "    lives = txt(Text.height 50) (toString player1.lives)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc4Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc5 =
  codeText "  in"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc5Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc6 =
  codeText "    container w h middle <|"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc6Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc7 =
  codeText "    collage gameWidth gameHeight"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc7Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc8 =
  codeText "      [ rect gameWidth gameHeight"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc8Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc9 =
  codeText "        |> filled elmGrey"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc9Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc10 =
  codeText "      , ngon 6 25 "
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc10Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc11 =
  codeText "        |> filled elmOrange"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc11Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc12 =
  codeText "        |> move (badGuy.x, badGuy.y)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc12Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc13 =
  codeText "      , circle 20"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc13Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc14 =
  codeText "        |> filled elmBlue"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc14Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc15 =
  codeText "        |> move (player1.x, player1.y)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc15Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc16 =
  codeText "      , toForm lives"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc16Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc17 =
  codeText "        |> move (0, gameHeight/2 - 40)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc17Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc18 =
  codeText "      , toForm (if state == Play then spacer 1 1"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc18Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc19 =
  codeText "        |> move (0, 40 - gameHeight/2)"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc19Msg) else (Signal.message hoveredOn.address ""))
...
viewFunc20 =
  codeText "      ]"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address viewFunc20Msg) else (Signal.message hoveredOn.address ""))
...
elmGreyColor =
  codeText "elmGrey = rgb 71 80 102"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address elmGreyColorMsg) else (Signal.message hoveredOn.address ""))
...
elmBlueColor =
  codeText "elmBlue = rgb 76 166 195"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address elmBlueColorMsg) else (Signal.message hoveredOn.address ""))
...
elmGreenColor =
  codeText "elmGreen = rgb 127 209 17"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address elmGreenColorMsg) else (Signal.message hoveredOn.address ""))
...
textColor =
  codeText "textColor = white"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address textColorMsg) else (Signal.message hoveredOn.address ""))
...
txtFunc1 =
  codeText "txt f = Text.fromString >> Text.color textColor >>"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address txtFunc1Msg) else (Signal.message hoveredOn.address ""))

txtFunc2Msg = "The text function also sets the font to monospace."
txtFunc2 : Element
txtFunc2 =
  codeText "&uArr; Text.monospace >> f >> leftAligned"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address txtFunc2Msg) else (Signal.message hoveredOn.address ""))
...
msgFunc1 =
  codeText "msg = 'SPACE to start, &larr ;&uarr ;&darr ;"
    |> hoverable (\ r -> if r then (Signal.message hoveredOn.address msgFuncMsg) else (Signal.message hoveredOn.address ""))
...
packageLink = a [href "http://package.elm-lang.org/", target "_blank"] [Html.text "Elm Package Catalog"]
exampleLink = a [href "http://elm-lang.org/Examples.elm", target "_blank"] [Html.text "Elm Examples"]
syntaxLink = a [href "http://elm-lang.org/learn/Syntax.elm", target "_blank"] [Html.text "Elm Syntax"]
mailingLink = a [href "https://groups.google.com/forum/?fromgroups#!forum/elm-discuss", target "_blank"] [Html.text "Elm Mailing List"]
twitterLink = a [href "https://twitter.com/elmlang", target "_blank"] [Html.text "Elm on Twitter"]
ircLink = a [href "http://webchat.freenode.net/?channels=elm", target "_black"] [Html.text "Elm on IRC"]
videoLink1 = a [href "http://elm-lang.org/learn/courses/beginner/Programming.elm", target "_blank"] [Html.text "Intro to Elm Video"]
videoLink2 = a [href "http://elm-lang.org/learn/courses/beginner/Graphics.elm", target "_blank"] [Html.text "Intro to Elm Graphics"]
videoLink3 = a [href "http://elm-lang.org/learn/courses/beginner/Lists-and-Records.elm", target "_blank"] [Html.text "Intro to Lists and Records"]
